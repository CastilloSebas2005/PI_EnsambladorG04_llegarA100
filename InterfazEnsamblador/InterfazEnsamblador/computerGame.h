...
namespace InterfazEnsamblador {
	...
	private:
		...
		void computerTurn() {
			...
			if (isWinner) {
				InterfazEnsamblador::ganadorhumano^ form = gcnew InterfazEnsamblador::ganadorhumano();
				form->ShowDialog();
				this->Close();
			}
			...
			counterF = this->counter;
			isWinner = checkCounter(&counterF);
			if (isWinner) {
				InterfazEnsamblador::ganadorComputadora^ form = gcnew InterfazEnsamblador::ganadorComputadora();
				form->Show();
				Close();
			}
		}
